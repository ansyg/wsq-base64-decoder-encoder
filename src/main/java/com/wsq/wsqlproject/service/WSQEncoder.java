...
import com.wsq.wsqlproject.service.WSQHelper.Ref;
import com.wsq.wsqlproject.service.WSQHelper.Token;
...
public class WSQEncoder implements WSQConstants, NISTConstants {
  ...
  public static void encode(final OutputStream os, final Bitmap bitmap, final double bitRate, final String... comments)
      throws IOException {
    encode(os, bitmap, bitRate, null, comments);
  }

  public static void encode(final DataOutput dataOutput, final Bitmap bitmap, final double bitRate,
      final String... comments) throws IOException {
    encode(dataOutput, bitmap, bitRate, null, comments);
  }

  public static void encode(final OutputStream os, final Bitmap bitmap, final double bitRate,
      final Map<String, String> metadata,
      final String... comments) throws IOException {
    encode((DataOutput) new DataOutputStream(os), bitmap, bitRate, metadata, comments);
  }

  public static void encode(final DataOutput dataOutput, final Bitmap _bitmap, final double bitRate,
      final Map<String, String> metadata,
      final String... comments) throws IOException {
    ...
    wsqDecompose(token, fdata, bitmap.getWidth(), bitmap.getHeight(), token.tableDTT.hifilt, MAX_HIFILT,
        token.tableDTT.lofilt, MAX_LOFILT);
    ...
    hufftable = gen_hufftable_wsq(token, huffbits, huffvalues, qdata, qsize1.get(),
        new int[] { qsize2.get(), qsize3.get() });
    ...
    compress_block(dataOutput, qdata, qsize1.get() + qsize2.get(), qsize3.get(), MAX_HUFFCOEFF, MAX_HUFFZRUN,
        hufftable);
   ...
   * Converts an image's unsigned character pixels to floating point values in the
   * range +/- 128.0.
   *
   ...
   */
  private static float[] convertImageToFloat(final byte[] data, final int width, final int height,
      final Ref<Float> m_shift, final Ref<Float> r_scale) {
    if (data == null) {
   ...
   */
  private static void wsqDecompose(final Token token, final float[] fdata, final int width, final int height,
      final float[] hifilt, final int hisz, final float[] lofilt, final int losz) {
  ...
  private static void getLets(final float[] newdata, /* image pointers for creating subband splits */
    ...
    int lopass, hipass; /*
                         * pointers of where to put lopass
   ...
   *
   * @param token  contains quant_vals quantization parameters and quantization
   *               "tree" and treelen. NOTE: This routine will write to
   *               <code>var</code> field inside <code>quant_vals</code>.
   ...
   *
   * @param token  contains quantization parameters, quantization tree, size of
   *               quantization tree
   ...
   */
  private static int[] quantize(final Token token, final AtomicInteger qsize, final float[] fip, final int width,
      final int height) {
   ...
   */
  private static void quant_block_sizes(final Token token, final AtomicInteger oqsize1, final AtomicInteger oqsize2,
      final AtomicInteger oqsize3) {
  ...
  private static void putc_huffman_table(final DataOutput dataOutput, final int marker, final int tableId,
      final int[] huffbits, final int[] huffvalues) throws IOException {
   ...
   * @throws IOException
   */
  private static void putc_frame_header_wsq(final DataOutput dataOutput, final int width, final int height,
      final float m_shift, final float r_scale) throws IOException {
   ...
   */
  private static void putc_transform_table(final DataOutput dataOutput, final float[] lofilt, final int losz,
      final float[] hifilt, final int hisz) throws IOException {
    ...
    for (coef = (losz >> 1); (coef & 0xFFFFFFFFL) < (long) losz; coef++) {
      ...
      if (dbl_tmp == 0.0) {
        ...
        throw new IllegalStateException(
            String.format("ERROR: putc_transform_table : lofilt[%d] to high at %f", coef, dbl_tmp));
      }
    ...
    for (coef = (hisz >> 1); (int) (coef & 0xFFFFFFFFL) < (long) hisz; coef++) {
      ...
      if (dbl_tmp == 0.0) {
        ...
        dbl_tmp = hifilt[(int) (coef & 0xFFFFFFFFL)];
        throw new IllegalStateException(
            "ERROR: putc_transform_table : hifilt[" + coef + "] to high at " + dbl_tmp + "");
      }
   ...
   */
  private static void putc_quantization_table(final DataOutput dataOutput, final Token token) /*
      ...
      if (sub >= 0 && sub < 60) {
        if (token.quant_vals.qbss[sub] != 0.0f) {
          ...
          if (flt_tmp < 65535) {
            ...
            flt_tmp = token.quant_vals.qbss[sub];
            throw new IllegalStateException(
                String.format("ERROR : putc_quantization_table : Q[%d] too high at %f", sub, flt_tmp));
          }
          ...
          if (flt_tmp < 65535) {
            ...
            throw new IllegalArgumentException(
                String.format("ERROR : putc_quantization_table : Z[%d] too high at %f", sub, flt_tmp));
          }
        } else {
          scale_ex = 0;
          scale_ex2 = 0;
   ...
   */
  private static void putc_nistcom_wsq(final DataOutput dataOutput, final Bitmap bitmap, final float r_bitrate,
      final Map<String, String> metadata, final String[] comments) throws IOException {
   ...
   * @param comment
   */
  private static void putc_comment(final DataOutput dataOutput, final int marker, final String comment)
      throws IOException {
   ...
   * @param block_sizes
   */
  private static WSQHelper.HuffCode[] gen_hufftable_wsq(final Token token, final Ref<int[]> ohuffbits,
      final Ref<int[]> ohuffvalues, final int[] sip, final int offset, final int[] block_sizes) {
    ...
    for (int i = 1; i < block_sizes.length; i++) {
      huffcounts2 = count_block(MAX_HUFFCOUNTS_WSQ, sip, offset + block_sizes[i - 1], block_sizes[i], MAX_HUFFCOEFF,
          MAX_HUFFZRUN);
  ...
  private static int[] count_block(
    ...
    int rcnt = 0, state; /*
                          * zero run count and if current pixel
   ...
   */
  private static WSQHelper.HuffCode[] build_huffsizes(final AtomicInteger temp_size, final int[] huffbits,
      final int max_huffcounts) {
