...
public class WSQDecoder implements WSQConstants, NISTConstants {
  ...
  public static void getCHuffmanTableWSQ(final DataInput DataInput, final Token token) throws IOException {
    ...
    while (bytesLeft != 0) {
      ...
      final WSQHelper.HuffmanTable huffmantable = getCHuffmanTable(DataInput, token, MAX_HUFFCOUNTS_WSQ, bytesLeft,
          false);
  ...
  private static WSQHelper.HuffmanTable getCHuffmanTable(final DataInput dataInput, final Token token,
      final int maxHuffcounts, int bytesLeft, final boolean readTableLen) throws IOException {
  ...
  private static int[] huffmanDecodeDataMem(final DataInput DataInput, final Token token, final int size)
      throws IOException {
    ...
    while (!isPrematureEOF && (marker.value != EOI_WSQ)) {
      ...
      try {
        ...
        final int nodeptr = decodeDataMem(DataInput, mincode, maxcode, valptr, token.tableDHT[hufftableId].huffvalues,
  ...
  private static void genDecodeTable(final WSQHelper.HuffCode[] huffcodeTable, final int[] maxcode, final int[] mincode,
      final int[] valptr, final int[] huffbits) {
    for (int i = 0; i <= MAX_HUFFBITS; i++) {
      maxcode[i] = 0;
      mincode[i] = 0;
      valptr[i] = 0;
  ...
  private static int decodeDataMem(final DataInput DataInput, final int[] mincode, final int[] maxcode,
      final int[] valptr, final int[] huffvalues, final WSQHelper.Ref<Integer> bitCount,
      final WSQHelper.Ref<Integer> marker, final WSQHelper.Ref<Integer> nextByte) throws IOException {

    short code = (short) getCNextbitsWSQ(DataInput, marker, bitCount, 1, nextByte); /*
                                                                                     * becomes a huffman code word (one
                                                                                     * bit at a time)
                                                                                     */
    if (marker.value != 0) {
      return -1;
    ...
    for (inx = 1; code > maxcode[inx]; inx++) {
      final int tbits = getCNextbitsWSQ(DataInput, marker, bitCount, 1, nextByte); /*
                                                                                    * becomes a huffman code word (one
                                                                                    * bit at a time)
                                                                                    */
  ...
  private static int getCNextbitsWSQ(final DataInput dataInput, final WSQHelper.Ref<Integer> marker,
      final WSQHelper.Ref<Integer> bitCount, final int bitsReq, final WSQHelper.Ref<Integer> nextByte)
      throws IOException {
    if (bitCount.value == 0) {
  ...
  private static void wsqReconstruct(final Token token, final float[] fdata, final int width, final int height) {
    ...
    for (int node = W_TREELEN - 1; node >= 0; node--) {
      ...
      // log.debug("{} {} {} {}",new Object[] {token.wtree[node].lenx,
      // token.wtree[node].leny,token.wtree[node].invcl,token.wtree[node].invrw});
  ...
  private static void joinLets(
    ...
    for (cl_rw = 0; cl_rw < len1; cl_rw++) {
      fhre = loop1(newdata, olddata, newIndex, oldIndex, pitch, stride, hi, hsz, lo, lsz, inv, cl_rw, da_ev, loc, hoc,
          hlen, llen, nstr, pstr, olle, ohle, olre, ohre, lotap, hotap, asym, fhre, ofhre, ssfac);
    }
  ...
  private static int loop1(
      final float[] newdata, final float[] olddata, final int newIndex, final int oldIndex, final int pitch,
      final int stride, final float[] hi, final int hsz, final float[] lo, final int lsz, final int inv,
      final int cl_rw, final int da_ev, final int loc, final int hoc, final int hlen, final int llen, final int nstr,
      final int pstr, final int olle, final int ohle, final int olre, final int ohre, final int lotap, final int hotap,
      final int asym, int fhre, final int ofhre, final float ssfac) {
    ...
    // ..... what on earth is this?! Why are we returning non-conditionally in the
    ...
    // This is what the above actually does. If it's not supposed to, I'll come back
    // and fix it.
    if (lstap <= 1) {
  ...
  private static int loop2(final float[] newdata, final float[] olddata, final int stride, final float[] lo,
      final int lsz, final int nstr, final int pstr, final int lp0, final int lp1, final int limg, int lpx, int lpxstr,
      int lle, int lre, final int tap) {
  ...
  private static byte[] convertImageToByte(final float[] img, final int width, final int height, float mShift,
      final float rScale) {
